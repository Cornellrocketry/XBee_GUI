    ...
    public void run() {
    	while (keepListening) {
			try {
				...
				if (response.getApiId() == ApiId.ZNET_RX_RESPONSE) {
					...
					if (packet.charAt(0)=='<'){
						...
						if (receiving){
							...
							Main.ne++;
							Main.addToReceiveText("Error (" + Main.ne + "): New packet before last data stream ended...");
							Main.nr++;
							Main.addToReceiveText("Incomplete Received (" + Main.nr + "): " + data);
							Main.nr++;
							Main.addToReceiveText("New Received (" + Main.nr + "): " + packet);
						}
						receiving=true;
					}else if (!receiving){
						Main.nr++;
						Main.addToReceiveText("Received (" + Main.nr + "): " + packet);
					}else{
						...
						if (packet.charAt(packet.length()-1)=='>'){
							...
							Main.nr++;
							Main.addToReceiveText("Received (" + Main.nr + "): " + data);
							receiving=false;
							data="";
				...
				Main.ne++;
				Main.addToReceiveText("Error (" + Main.ne + "): XBee Problem: " + e.getMessage());
				e.printStackTrace();
			}
		}
    }
