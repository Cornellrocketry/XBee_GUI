...
public class Main {
	...
	static int[][] addr = {
			{0, 0x13, 0xa2, 0, 0x40, 0xbf, 0x56, 0x47},
			{0, 0x13, 0xa2, 0, 0x40, 0xbf, 0x56, 0xa5},
			{0, 0x13, 0xa2, 0, 0x40, 0x91, 0x79, 0xa7},
			{0, 0x13, 0xa2, 0, 0x40, 0x91, 0x79, 0x6f}
	};
	
	// final XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0x91, 0x79, 0xa7);
	//final XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0x91, 0x79, 0x6f);

	//new xbees:
	//small cable:
	//final XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0xbf, 0x56, 0xa5);

	//long cable:
	//final XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0xbf, 0x56, 0x47);
	...
	public static void updateAddr(){
        ...
        selAddr = addr[idx];
		addr64 = new XBeeAddress64(selAddr[0], selAddr[1],selAddr[2],selAddr[3],selAddr[4],selAddr[5],selAddr[6],selAddr[7]);
	}
	...
	initXBeeButton.addActionListener(new ActionListener(){
		public void actionPerformed(ActionEvent e){
			try {
				...
				addToReceiveText("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -"+ System.getProperty("line.separator"));
			} catch (XBeeException e1) {
				ne++;
				addToReceiveText("Error (" + ne + "): Could not connect to XBee :( make sure port isn't being used by another program (including this one)!");
			}
		}
	});
	...
	btn.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			/*
	            	nr= nr+1;
	        		addToReceiveText("Received (" + nr + "): " + sendEdit.getText(),receiveText);
			 */
	...
	/*
		p2.add(new JLabel("Recieved Packet: "));
		packetLabel= new JLabel("");
		p2.add(packetLabel);
